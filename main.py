...
class Patient(BaseModel):
    ...
    gender: Annotated[Literal["Male", "Female", "Others", "male", "female"], Field(..., description = "Gender of the patient")]
...
class PatientUpdate(BaseModel):
    ...
    gender: Annotated[Optional[Literal["male", "female", "Male", "Female", "Others"]], Field(default=None)]
...
def create_patient(patient: Patient):
    ...
    #check if patient already present or not
    if patient.id in data:
        raise HTTPException(status_code = 400, detail = "Patient already exists")
    
    #add the patient in the data
    data[patient.id] = patient.model_dump(exclude = {"id"})

    #save the data back into the json file
    save_data(data)
